...
function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-[#E8EDE9] h-screen sticky top-0">
      <div className="p-6">
        <Link href="/" className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-[#00A86B] flex items-center justify-center">
            <GraduationCap className="w-4 h-4 text-white" />
          </div>
          <span className="text-sm font-bold text-[#071C1A]">GFF AI</span>
        </Link>
      </div>
        ...
        {links.map((link) => (
          <a
            ...
            className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${
              link.active
                ? "bg-[#DFFBF1] text-[#008F5A]"
                : "text-[#5A6B69] hover:bg-[#F0F4F2] hover:text-[#071C1A]"
            }`}
          >
      ...
      <div className="p-4 border-t border-[#E8EDE9]">
        <a
          href="/"
          className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-[#5A6B69] hover:bg-[#F0F4F2] transition-all"
        >
...
function DashboardHeader() {
  const tierColors = {
    ...
    Silver: "bg-[#F0F4F2] text-[#475569] border-[#CBD5E1]",
    ...
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#DFFBF1] to-[#E6F1FF] flex items-center justify-center text-lg font-bold text-[#071C1A]">
            {studentProfile.name
              .split(" ")
              .map((n) => n[0])
              .join("")}
          ...
          <div>
            <h2 className="text-lg font-bold text-[#071C1A]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="relative w-9 h-9 rounded-xl bg-[#F8FAF9] border border-[#E8EDE9] flex items-center justify-center hover:bg-[#F0F4F2] transition-colors">
            <Bell className="w-4 h-4 text-[#5A6B69]" />
      ...
      {/* Quick Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-[#E8EDE9]">
        {[
          ...
          <div key={stat.label} className="p-3 rounded-xl bg-[#F8FAF9]">
            <div className="flex items-center gap-1.5 mb-1">
              <stat.icon className="w-3 h-3 text-[#8B9A97]" />
              <span className="text-[10px] text-[#8B9A97] uppercase tracking-wider">
                {stat.label}
              </span>
            </div>
            <p className="text-sm font-semibold text-[#071C1A] truncate">
              {stat.value}
            </p>
          </div>
        ))}
...
function ActionCard() {
  return (
    <motion.div
      ...
      className="bg-gradient-to-r from-[#00A86B] to-[#008F5A] rounded-2xl p-5 text-white"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <button className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white text-[#00A86B] text-xs font-semibold hover:bg-white/90 transition-colors">
            Complete Now
...
function JourneyTimeline() {
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-[#E8EDE9] p-6"
    >
      <h3 className="text-base font-bold text-[#071C1A] mb-4 flex items-center gap-2">
      ...
      <div className="space-y-2">
        {curriculum.map((module) => {
          ...
          return (
            <div
              key={module.id}
              className="border border-[#E8EDE9] rounded-xl overflow-hidden"
            >
              <button
                onClick={() =>
                  setOpenModule(openModule === module.id ? null : module.id)
                }
                className="w-full flex items-center justify-between p-4 text-left hover:bg-[#FAFCFB] transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <h4 className="text-sm font-semibold text-[#071C1A]">
                      {module.title}
                    </h4>
                    <span className="text-[10px] text-[#5A6B69] bg-[#F0F4F2] px-2 py-0.5 rounded-full">
                      {completed}/{total}
                    </span>
                  </div>
                ...
                {openModule === module.id ? (
                  <ChevronUp className="w-4 h-4 text-[#5A6B69] ml-3" />
                ) : (
                  <ChevronDown className="w-4 h-4 text-[#5A6B69] ml-3" />
                )}
              </button>

              {openModule === module.id && (
                <div className="px-4 pb-4 border-t border-[#E8EDE9]">
                    ...
                    {module.topics.map((topic) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-[#8B9A97]">
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        {[
          { label: "Attendance", value: "92%", color: "bg-[#DFFBF1] text-[#008F5A]" },
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#E8EDE9]">
              <th className="text-left py-2 text-[#8B9A97] font-medium">
                Assessment
              </th>
              <th className="text-center py-2 text-[#8B9A97] font-medium">
                Score
              </th>
              <th className="text-center py-2 text-[#8B9A97] font-medium">
                Status
          ...
          <tbody>
            {assessments.map((a) => (
              <tr key={a.id} className="border-b border-[#F0F4F2]">
                <td className="py-2.5 text-[#071C1A]">{a.name}</td>
                <td className="py-2.5 text-center">
                  {a.status === "completed" ? (
                    <span className="font-semibold text-[#071C1A]">
                      {a.score}/{a.maxScore}
                    </span>
                  ) : (
      ...
      <div className="space-y-2">
        {checklist.map((item) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-[#071C1A]">{item.label}</p>
            </div>
            <span
